...
local function target_location() --标靶信息传入模块
    if param:get("TARGET_GET") == 1 then
        ...
        if target_num == 1 then
            itargetloc = {38.556825,115.138644} --{纬度,经度}
        elseif target_num == 2 then
            itargetloc = {38.5570003,115.138662}
        elseif target_num == 3 then
            itargetloc = {38.55703,115.138429}
        elseif target_num == 4 then
            itargetloc = {38.556836,115.138412}
        end
        return true
    else
        return false
...
local function dropping_calculation() --投弹计算
    ...
    if relative_height <= 0 or velocity_vec:length() < 5 then
        return false
    end
    local g = 9.7997 --河北重力加速度
    ...
    if math.abs(remaining_distance) < 5 then --投弹决策范围3米
        return true
    else
        return false
    end
